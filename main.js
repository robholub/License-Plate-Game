...
export default function App() {
  ...
  return (
    // Added select-none, touch-manipulation, and 100dvh for Android App behavior
    <div 
    ...
    >
      ...
      {appState === 'PLAYING' && (
        <GameScreen 
          ...
          onClaim={handleClaimPlate}
          onAddCustom={handleAddCustomPlate}
          getScore={getScore}
          onFinish={finishGame}
        />
      )}
    ...
    <div className="max-w-md mx-auto p-6 pt-12 min-h-[100dvh] flex flex-col">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 mb-6 flex-1 overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-3">
          {players.map((player, index) => (
            <div key={player.id} className="flex items-center gap-3">
              <div 
                className="w-4 h-8 rounded-full shadow-inner flex-shrink-0"
              ...
              {players.length > 1 && (
                <button 
                  ...
                  className="p-3 text-slate-400 hover:text-red-500 hover:bg-red-50 active:bg-red-100 rounded-xl transition-colors"
        ...
        {players.length < 8 && (
          <button 
            onClick={onAddPlayer}
            className="w-full mt-4 flex items-center justify-center gap-2 py-3 border-2 border-dashed border-slate-200 text-slate-500 rounded-xl hover:bg-slate-50 active:bg-slate-100 transition-colors font-medium"
          >
            <Plus size={20} />
            Add Player
          </button>
      ...
      <button 
        onClick={onStart}
        className="w-full bg-blue-600 active:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-blue-200 flex items-center justify-center gap-2 transition-transform active:scale-[0.98] mt-auto mb-6"
      >
...
// ==========================================
function GameScreen({ 
  players, plates, activePlayerId, setActivePlayerId, 
  onClaim, onAddCustom, getScore, onFinish 
}) {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredPlates = useMemo(() => {
    return plates.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [plates, searchQuery]);
    ...
    <div className="pb-24 flex flex-col min-h-[100dvh]">
      <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm pt-2">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button 
            onClick={onFinish}
            className="text-sm font-bold bg-slate-100 active:bg-slate-200 text-slate-700 px-4 py-2 rounded-full transition-colors flex items-center gap-2"
          >
            <Flag size={16} /> Finish
          </button>
        </div>
          ...
          {players.map(player => {
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-2xl transition-all border ${
                  isActive 
                    ? 'border-transparent shadow-md transform scale-105' 
                    : 'border-slate-200 bg-white text-slate-600 active:bg-slate-50'
                }`}
                style={{ 
                  backgroundColor: isActive ? player.color : undefined,
                  color: isActive ? '#fff' : undefined
                }}
              >
                <span className="font-bold whitespace-nowrap">{player.name}</span>
                <span className={`text-xs font-black px-2 py-0.5 rounded-full ${isActive ? 'bg-white/30' : 'bg-slate-100 text-slate-500'}`}>
                  {score}
                </span>
              </button>
            );
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 py-6">
        <div className="relative mb-6">
          ...
          {searchQuery && (
            <button 
              ...
              className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 active:text-slate-600"
            >
              <X size={20} />
            </button>
          )}
        ...
        <div className="mb-4 flex justify-between items-end">
          <h2 className="text-lg font-extrabold text-slate-800">
            {searchQuery ? 'Search Results' : 'All Plates'}
          </h2>
          <span className="text-sm text-slate-500 font-medium">
            {plates.filter(p => p.claimedBy !== null).length} / {plates.length} Claimed
          </span>
        </div>
        ...
        {filteredPlates.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {filteredPlates.map(plate => {
              ...
              return (
                <button
                  key={plate.id}
                  onClick={() => onClaim(plate.id)}
                  className={`
                    relative p-4 rounded-2xl text-left transition-all duration-200 active:scale-95 flex flex-col justify-between min-h-[110px]
                    ${isClaimed 
                      ? 'shadow-sm border border-transparent' 
                      : 'bg-white border border-slate-200 shadow-sm active:border-blue-300 active:shadow-md'
                    }
                  `}
                  style={{
                    backgroundColor: isClaimed ? owner?.color : undefined,
                    color: isClaimed ? '#fff' : undefined
                  }}
                >
                  <div className="flex justify-between items-start w-full gap-2">
                    ...
                    {plate.flagCode ? (
                      <div className={`flex-shrink-0 w-8 h-6 rounded shadow-sm overflow-hidden flex items-center justify-center ${isClaimed ? 'bg-white/20' : 'bg-slate-100 border border-slate-200'}`}>
                        <img 
                          src={`https://flagcdn.com/w40/${plate.flagCode}.png`}
                          srcSet={`https://flagcdn.com/w80/${plate.flagCode}.png 2x`}
                          alt={`${plate.name} flag`}
                      ...
                      <div className={`flex-shrink-0 w-8 h-6 rounded flex items-center justify-center ${isClaimed ? 'text-white/60' : 'text-slate-300 bg-slate-50 border border-slate-200'}`}>
                        <Map size={14} />
                      </div>
                    )}
                  </div>
                  ...
                  {isClaimed && (
                    <div className="mt-4 flex items-center justify-between">
                      <span className="text-xs font-semibold opacity-90 truncate pr-2">
                        {owner?.name}
                      </span>
                      {isOwnedByActive ? (
                        <span className="bg-white/20 p-1 rounded-full flex-shrink-0"><X size={12} /></span>
                      ) : (
                        <span className="bg-black/10 p-1 rounded-full flex-shrink-0"><Check size={12} /></span>
                      )}
                    </div>
                  )}
          ...
          <div className="text-center py-12 bg-white rounded-3xl border border-slate-200 shadow-sm">
            ...
            <button
              ...
              className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold active:bg-slate-800 transition-colors flex items-center gap-2 mx-auto"
            >
    ...
    <div className="min-h-[100dvh] flex flex-col max-w-2xl mx-auto p-4 sm:p-6 pb-12">
      ...
      <div className="text-center py-10">
        ...
        <p className="text-slate-500 text-lg">
          The drive is over. Here are the final results.
        </p>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mb-8 flex-1">
        {rankedPlayers.map((player, index) => {
          ...
          return (
            <div 
              key={player.id} 
              className={`p-6 ${index !== rankedPlayers.length - 1 ? 'border-b border-slate-100' : ''}`}
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                      {player.name}
                      {index === 0 && !isTie && <Award size={18} className="text-yellow-500" />}
                    </h2>
                    <p className="text-sm text-slate-500">{score} Plates Claimed</p>
                  </div>
                </div>
                <div 
                  className="text-2xl font-black px-4 py-2 rounded-xl text-white shadow-sm"
                  style={{ backgroundColor: player.color }}
                >
                  {score}
                </div>
              ...
              {score > 0 ? (
                <div className="flex flex-wrap gap-2 mt-4">
                  {playerPlates.map(plate => (
                    <span 
                      key={plate.id}
                      className="text-xs font-semibold px-2.5 py-1 rounded-md bg-slate-50 text-slate-600 border border-slate-200 flex items-center gap-1.5"
                    >
                      {plate.flagCode ? (
                        <img 
                          src={`https://flagcdn.com/w20/${plate.flagCode}.png`}
                          alt=""
      ...
      <div className="mt-auto">
        <button 
          onClick={onRestart}
          className="w-full bg-slate-900 active:bg-slate-800 text-white font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-[0.98]"
        >
